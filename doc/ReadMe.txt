...
svn revision numbers as well as patch of the uncommited changes on both
repositories:
...
~/code/tectomt. Furthermore, uncommited changes to the repositories are
also saved in the form of a unified diff, allowing us to recover the
current source code in full extent. WARNING: only files already tracked
by mercurial and SVN will be included in the unified diff of every
snapshot, ie, all files appearing with a question mark when you issue
the commands hg status or svn status WILL NOT be included in the diff.
