...
function w2a {
    create_dir atrees lemmas batches
    find batches -name "w2a_*" -delete
    rm -f todo.w2a
    comm -23 \
        <(sed 's/$/.streex/' corpus/parts.txt) \
        <(find atrees -name '*.streex' -printf '%f\n' | sort) \
        > todo.w2a
    if test -s todo.w2a; then
        split -d -a 3 -n l/$num_procs todo.w2a batches/w2a_
        rm -f todo.w2a
    fi
    batches=$(find batches -name "w2a_*" -printf '%f\n')
