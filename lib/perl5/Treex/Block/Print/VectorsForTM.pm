...
has wsd_features => (
    ...
    isa           => 'Bool',
    default       => 0,
    documentation => 'Print WSD synsets',
);
...
sub print_tnode_features {
    ...
    # WSD
    if ($self->wsd_features) {
        my $src_anode = $src_tnode->get_lex_anode();
        if (defined $src_anode->wild->{lx_wsd}) {
            my $synsetid = $src_anode->wild->{lx_wsd};
            push @add_features, "synsetid=$synsetid";
        }
    }
...
Luís Gomes <luis.gomes@di.fc.ul.pt> (added WSD features)
